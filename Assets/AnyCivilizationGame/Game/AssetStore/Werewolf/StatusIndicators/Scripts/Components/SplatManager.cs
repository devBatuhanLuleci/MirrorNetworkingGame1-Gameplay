...
namespace Werewolf.StatusIndicators.Components
{
    ...
    public class SplatManager : MonoBehaviour
    {
        ...
        {
            SpellIndicator[] spellIndicators = SpellIndicators;
            return SpellIndicators.Where(x => x.name.Replace(" ", String.Empty) == splatName.Replace(" ", String.Empty)).FirstOrDefault();
        }
