...
public class Reporter : MonoBehaviour
{
    ...
    public UnityEventBool OnActiveChangeInvertedEvent;
    void Awake()
	{
		if (!Initialized)
			Initialize();
	...
	{
		OnActiveChangeInvertedEvent.AddListener((isNotActive) => gameObject.SetActive(!isNotActive));
	...
	void drawToolBar()
	{
		...
		if (GUILayout.Button(closeContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2))) {
            ...
            OnActiveChangeInvertedEvent?.Invoke(true);
		}
	...
	void doShow()
	{
		...
		OnActiveChangeInvertedEvent?.Invoke(false);
	}
